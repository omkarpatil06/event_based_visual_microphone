...
class VAE(Model):
    ...
    def build_encoder(self, input_shape):
        ...
        x = self.encoder_layer(input_layer, 16, 3, 2, 'same')
        x = self.encoder_layer(x, 32, 5, 2, 'same')
        x = self.encoder_layer(x, 64, 5, 2, 'same')
        x = self.encoder_layer(x, 64, 5, 2, 'same')
        x = self.encoder_layer(x, 64, 3, 2, 'same')
    ...
    def build_decoder(self):
        """Constructs the decoder part of VAE."""
        decoder_input = layers.Input(shape=(32,))
        x = layers.Dense(64 * 4 * 4)(decoder_input)
        x = layers.Reshape((4, 4, 64))(x)
        x = self.decoder_layer(x, 64, 3, 2, 'same')
        x = self.decoder_layer(x, 64, 5, 2, 'same')
        x = self.decoder_layer(x, 64, 5, 2, 'same')
        x = self.decoder_layer(x, 32, 5, 2, 'same')
        x = self.decoder_layer(x, 16, 3, 2, 'same')
        output_upsampling = layers.Conv2DTranspose(1, 3, 2, 'same')(x)
